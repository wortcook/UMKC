...
Creates visualizations and markdown report from k-mer analysis results.
"""
...
print("Loading k-mer analysis data...")
kmer_data = {}
kmer_stats = {}
...
for k in k_values:
    ...
    if os.path.exists(kmer_path):
        try:
            ...
            print(f"  k={k}: {len(df):,} unique k-mers, total occurrences: {df['count'].sum():,}")
...
for k in k_values:
    if k in kmer_stats:
        ...
        coverages.append(min(coverage, 100))  # Cap at 100%
...
# Analyze homopolymer runs in top k-mers
homopolymer_data = []
for k in k_values:
    if k in kmer_data:
        df = kmer_data[k]
