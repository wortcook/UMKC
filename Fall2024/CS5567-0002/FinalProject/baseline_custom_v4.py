...
#Gather all the trainable layers
for layer in inner_model.layers:
    if type(layer) == layers.Conv2D:
        trainable_layers.append(layer)
    elif type(layer) == layers.BatchNormalization:
        trainable_layers.append(layer)

#initialize the trainable layers to False
...
for layer in trainable_layers:
    ...
    optimizer_adam = optimizers.SGD()

    #We have to recompile the model after changing the trainable layers
    headed_model.compile(optimizer=optimizer_adam, loss='categorical_crossentropy', metrics=['accuracy'], run_eagerly=True)

    result = headed_model.fit(ix_train, iy_train, epochs=1, validation_data=(ix_val,iy_val))

    #pickle the results
    import pickle

    with open(f'baseline_custom_v4_hist_{idx}.pkl', 'wb') as f:
        pickle.dump(result, f)

    idx += 1

#Run final fit with new head
headed_model = models.Sequential()
headed_model.add(inner_model)
headed_model.add(layers.Flatten())
headed_model.add(layers.Dense(64, activation=dense_activation))
...
with open(f'baseline_custom_v4_hist_final.pkl', 'wb') as f:
    pickle.dump(result, f)


inner_model.save('baseline_custom_head_v4.keras')
